...
def save_well(request):
    conn = pymssql.connect(
        "192.168.18.137:1433", "web_user", "Hyd@1234", "Mother_Database"
    )
    try:
        with conn.cursor() as cur:
            ...
            for i in rows:
                if SasWellName.objects.filter(name=i[0]).exists():
                    ...
                    if (
                        SasRigName.objects.filter(saswellname=well, name=i[1]).exists()
                        == False
                    ):
...
def equipmentLog(request):
    try:
        ...
        if request.method == "POST":
            ...
            form_hsd_formset_class = formset_factory(
                HsdForm, extra=request.POST.get("hsd_form-TOTAL_FORMS")
            )
            ...
            equip_formset_class = formset_factory(
                EquipmentForm, extra=request.POST.get("equip_form-TOTAL_FORMS")
            )
            ...
            equip_serv_formset_class = formset_factory(
                EquipServiceForm, extra=request.POST.get("equip_serv-TOTAL_FORMS")
            )
            ...
            down_formset_class = formset_factory(
                RigDownForm, extra=request.POST.get("rig_down-TOTAL_FORMS")
            )
            ...
            send_mail(
                ...
                "Hello Sir, An Equipment form has been submitted recently. Its details can be accessed through the link : https://commonanalytics.ongc.co.in/mechanical/view-equipment-log/"
                + str(rig.id)
                + "/",
                "gaurav@ongc.co.in",
        ...
        # error = ErrorLog(
        #     user_name=request.user, function_name="Equipment_Log", error_desc=str(e)
        # )
        # error.save()
        # send_mail(
        #     "Equipment_Log Error",
...
def viewEquipmentLog(request, rig_id):
    try:
        rig = Rig.objects.select_related(
            "equipment_singlehit",
            "service_singlehit",
            "rigdown_singlehit",
            "hsdbalance_singlehit",
        ).get(pk=rig_id)
        ...
        # error = ErrorLog(
        #     user_name=request.user, function_name="View_Equipment", error_desc=str(e)
        # )
        # error.save()
        # send_mail(
...
def reportEquipmentLog(request):
    try:
        if request.method == "POST":
            ...
            rig1 = (
                equipments1
            ) = hsdbalances1 = services1 = rigdowns1 = Rig.objects.filter(
                date__range=["2021-11-10", "2021-11-11"]
            ).all()
            ...
            for i in range(len(range1)):
                ...
                equipments = Equipment.objects.filter(
                    equip_user=request.user, today_equip=range1[i]
                ).all()
                equipments1 = chain(equipments1, equipments)
                hsdbalances = HSD_balance.objects.filter(
                    hsd_user=request.user, today_hsdbal=range1[i]
                ).all()
                hsdbalances1 = chain(hsdbalances1, hsdbalances)
                services = EquipmentService.objects.filter(
                    service_user=request.user, today_equip_service=range1[i]
                ).all()
                services1 = chain(services1, services)
                rigdowns = RigDown.objects.filter(
                    rigdown_user=request.user, today_rigdown=range1[i]
                ).all()
        ...
        #     function_name="reportEquipmentLog",
        #     error_desc=str(e),
        # )
        # error.save()
        # send_mail(
...
def electricalLog(request):
    ...
    if request.method == "POST":
        ...
        electrical_shift_formset_class = formset_factory(
            ElectricalShiftForm,
            extra=request.POST.get("electric_form-TOTAL_FORMS"),
        )
        data_dict = request.POST.dict()
        data_dict["form-TOTAL_FORMS"] = request.POST.get(
            "electric_form-TOTAL_FORMS"
        )
        electrical_shift_formset = electrical_shift_formset_class(data_dict)

        form = ElectricalRigForm(request.POST)

        if form.is_valid() and electrical_shift_formset.is_valid():
            rig = form.save(commit=False)
            ...
            messages.error(request, "Rig or Equipment details are not valid")
            # form.cleaned_data['date'] = datetime.today().strftime("%d%m%Y%H%M%S")
            print(request.POST["initial-date"])

        # send_mail(
        #     "Electrical Form Details",
        #     "Hello Sir, An Electrical form has been submitted recently. Its details can be accessed through the link : https://commonanalytics.ongc.co.in/mechanical/view-electric-log/"
        #     + str(rig.id)
        #     + "/",
        #     "gaurav@ongc.co.in",
        #     ["gaurav.jain@inspiraenterprise.com", request.session["receiver"]],
...
def viewElecticLog(request, rig_id):
    try:
        ...
        # error = ErrorLog(
        #     user_name=request.user, function_name="viewElecticLog", error_desc=str(e)
        # )
        # error.save()
        # send_mail(
    ...
    try:
        if request.method == "POST":
            fromdate = datetime.strptime(
                request.POST.get("From_electric"), "%Y-%m-%d"
            ).date()
            todate = datetime.strptime(
                request.POST.get("To_electric"), "%Y-%m-%d"
            ).date()
            ...
            date__range = [fromdate, todate]
            print(date__range)
            rig1 = electric_s1 = ElectricalRig.objects.filter(
                date__range=["2021-11-10", "2021-11-11"]
            ).all()

            for i in range(len(range1)):
                rig = ElectricalRig.objects.filter(
                    user_electric=request.user, today_rig_electric=range1[i]
                ).all()
                rig1 = chain(rig1, rig)
                print(rig1)
                electric_s = ElectricalShift.objects.filter(
                    electric_user=request.user, electric_today=range1[i]
                ).all()
        ...
        # error = ErrorLog(
        #     user_name=request.user,
        #     function_name="Report_Equipment_Log",
        #     error_desc=str(e),
        # )
        # error.save()
        # send_mail(
...
def drillLog(request):
    try:
        ...
        if request.method == "POST":
            ...
            drill_shift_formset_class = formset_factory(
                DrillShiftForm, extra=request.POST.get("drill_shift_form-TOTAL_FORMS")
            )
            data_dict = request.POST.dict()
            data_dict["form-TOTAL_FORMS"] = request.POST.get(
                "drill_shift_form-TOTAL_FORMS"
            )
            ...
            drill_mud_chemical_formset_class = formset_factory(
                DrillMudChemicalReportForm,
                extra=request.POST.get("drill_mud_chemical_form-TOTAL_FORMS"),
            )
            data_dict1 = request.POST.dict()
            data_dict1["form-TOTAL_FORMS"] = request.POST.get(
                "drill_mud_chemical_form-TOTAL_FORMS"
            )
            ...
            send_mail(
                ...
                "Hello Sir, A Drilling form has been submitted recently. Its details can be accessed through the link : https://commonanalytics.ongc.co.in/mechanical/view-drill-log/"
                + str(rig.id)
                + "/",
                "gaurav@ongc.co.in",
                ["gaurav.jain@inspiraenterprise.com", request.session["receiver"]],
        ...
        # error = ErrorLog(
        #     user_name=request.user, function_name="Drill_Log", error_desc=str(e)
        # )
        # error.save()
        # send_mail(
        #     "Drill_Log Error",
...
def viewDrillLog(request, rig_id):
    try:
        ...
        # error = ErrorLog(
        #     user_name=request.user, function_name="View_Drill_Log", error_desc=str(e)
        # )
        # error.save()
        # send_mail(
...
def reportDrillLog(request):
    try:
        ...
        if request.method == "POST":
            ...
            fromdate = datetime.strptime(
                request.POST.get("From_drill"), "%Y-%m-%d"
            ).date()
            ...
            rig1 = (
                ...
                drilldata1
            ) = drillmud1 = drillvol1 = drillsolid1 = DrillRig.objects.filter(
                date__range=["2021-11-10", "2021-11-11"]
            ).all()

            for i in range(len(range1)):
                rig = DrillRig.objects.filter(
                    user_drill=request.user, today_rig_drill=range1[i]
                ).all()
                rig1 = chain(rig1, rig)
                print(rig1)
                drillshift = DrillShift.objects.filter(
                    drill_shift_user=request.user, drill_shift_equip=range1[i]
                ).all()
                drillshift1 = chain(drillshift1, drillshift)
                drilllaboratory = DrillLaboratory.objects.filter(
                    drill_lab_user=request.user, drill_lab_service=range1[i]
                ).all()
                drilllaboratory1 = chain(drilllaboratory1, drilllaboratory)
                hydraulic = HydraulicData.objects.filter(
                    hyd_data_user=request.user, hyd_data_rigdown=range1[i]
                ).all()
                hydraulic1 = chain(hydraulic1, hydraulic)
                drilldata = Drilldata.objects.filter(
                    drill_data_user=request.user, drill_data_hsdbal=range1[i]
                ).all()
                drilldata1 = chain(drilldata1, drilldata)
                drillmud = DrillMudChemicalReport.objects.filter(
                    drill_mud_user=request.user, drill_mud_hsdbal=range1[i]
                ).all()
                drillmud1 = chain(drillmud1, drillmud)
                drillvol = DrillMudVolume.objects.filter(
                    drill_vol_user=request.user, drill_vol_hsdbal=range1[i]
                ).all()
                drillvol1 = chain(drillvol1, drillvol)
                drillsolid = DrillSolidControl.objects.filter(
                    drill_solid_user=request.user, drill_solid_hsdbal=range1[i]
                ).all()
        ...
        # error = ErrorLog(
        #     user_name=request.user, function_name="Report_Drill_Log", error_desc=str(e)
        # )
        # error.save()
        # send_mail(
